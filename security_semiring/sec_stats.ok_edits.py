...
fields = ['Table', 'Ave_C_Mean', 'Ave_C_RMS']
...
for tb in tbs:
    ...
    random.seed(6980677074433906231)
    for i in range(1,10):
        ...
        with open(file_name, mode='a') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=fields)
            writer.writerow({'Table': tb, 'Ave_C_Mean':str(c_mean), 'Ave_C_RMS':str(c_rms)})

    print('Finished writing {}.'.format(tb))
